...
const Event = () => {
	...
	return (
		<Box sx={{ display: 'grid', gridTemplateAreas: '"header""content"' }}>
			<Box sx={{ gridArea: 'header', display: 'grid', gridTemplateRows: '1fr 1fr' }}>
				<Image src="/blekksprut2.jpg" alt="noe alt-tekst" width={375} height={230} />
				<Box sx={{ background: 'linear-gradient(0deg, black, transparent)' }}>
					<IconButton>
						<NavigateBefore />
					</IconButton>
			...
			<Box sx={{ gridArea: 'content' }}>
				<Box>
					...
					<Typography component="em" sx={{color:'orangered'}}>System: Mage - the ascension</Typography>
				</Box>
                <Box sx={{display:'flex', gap:'.5em'}}>
                    {arrayet.map((vesen) => ( <Chip variant='outlined' label={vesen} key={vesen} icon={<NavigateBefore />} />))}
                </Box>
