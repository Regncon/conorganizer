...
const LoadingAssignedGameWrapper = ({ children }: PropsWithChildren<Props>) => {
    ...
    return isLoading ?
            <Box sx={{ display: 'grid', placeContent: 'center', alignItems: 'center', marginBlockStart: '2rem' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    <Typography>Venleg vent medan påmeldinga vert lasta inn...</Typography> <CircularProgress />
                </Box>
            </Box>
        :   children;
};

export default LoadingAssignedGameWrapper;
